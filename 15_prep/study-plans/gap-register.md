...
| Chủ đề | Ưu tiên | Trạng thái | Hiện có ở đâu | Ghi chú / nên bổ sung |
|--------|---------|-----------|---------------|----------------------|
| **Memory model & lock-free** (atomic ordering, happens-before, ABA, hazard pointer) | 🟠 | ✅ **đã lấp** | **Ch. 5** ([05-memory-model](../../16-book-summaries/cpp-concurrency/05-memory-model.md) — happens-before, synchronizes-with, 6 memory_order, CAS, fences) + **Ch. 7** ([07-lock-free-structures](../../16-book-summaries/cpp-concurrency/07-lock-free-structures.md) — lock-free vs wait-free, 3 kỹ thuật reclamation, hazard pointer, **ABA problem**) | Trước đây **không tài liệu nào trong repo phủ**. Đã lấp trọn bằng ch. 5 + ch. 7 của C++ Concurrency in Action. |
