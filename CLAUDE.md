...
- ⚖️ **Luật ƯU TIÊN khi hai file mâu thuẫn** (đầu `config.md`): người dùng nói trong phiên > **`config.md`** > `interview-types`/`tracks`/`bank/README` > `study-plans`. Sinh ra từ hai sự cố thật (13/08 luật `rapid`; 17/08 plan nói ngược config §4). **Phát hiện mâu thuẫn ⇒ sửa file tầng thấp cho khớp tầng cao, và ghi rõ VÌ SAO** — sửa im lặng thì lần sau có người sửa ngược lại.
- 📊 **[config.md §7 — ĐỘ PHỦ](14-prep/mock-interview/config.md) (BẮT BUỘC).** Mỗi 5 phiên / trước mỗi giai đoạn mới phải **đo phủ bank theo domain** bằng lệnh, ghi vào §📍 plan. Lý do: 17/08 đo tình cờ thấy phủ **29%** sau 15/28 ngày, với **`DRV` 0%** dù là trụ lớn nhất của JD — lỗi **chỉ nhìn thấy ở mức tổng**, không phiên đơn lẻ nào lộ ra.
- 📈 **Câu điểm thấp KHÔNG đi thẳng lên T2.** 0–2 điểm ⇒ đọc lại tài liệu rồi hỏi lại **T1 góc khác**; chỉ câu **3–4** mới đưa vào phiên đào sâu. Hỏi T2 trên nền T1 trống thì không đo được gì và không dạy được gì.
- ⚠️ **[config.md §6 — Hợp đồng ĐỘ SÂU](14-prep/mock-interview/config.md) (BẮT BUỘC đọc trước mọi phiên).** Chặn **hai lỗi ngược nhau**:
  - **Chống phiên NÔNG** (luật ①–⑥): **mọi câu** (mới / weak / retention) đều phải có **phần nền + follow-up mở rộng**; nguồn câu chỉ đổi *trọng số* (câu weak: nén nền còn 1 checkpoint, điểm gần như hoàn toàn từ follow-up). Kèm: hỏi qua snippet cụ thể thay vì định nghĩa, bắt viết code rồi review chính code đó, biên dịch chạy thật trước khi khẳng định.
  - **Chống phiên LỆCH TẦNG** (Trần độ sâu): ba tầng **T1 cơ chế · T2 vận dụng/đánh đổi · T3 chuyên sâu** (tên lệnh, flag, internals, lock-free). **Trần mặc định = T2** cho mọi loại phiên; chỉ `deep-dive` lên T3. **T3 hỏi được nhưng KHÔNG tính điểm** ở phiên mặc định — thiếu T3 vẫn đạt 4 nếu T1+T2 chắc. Bài coding: nhỏ 10–15′ · vừa 20–30′ · lớn 40′+ (chỉ `deep-dive`).
  - Phép thử phân biệt T2/T3: *"không biết thứ này thì có ra quyết định SAI trong công việc không?"* — Có ⟹ T2 (chấm). Không ⟹ T3 (không chấm).
- **Retention có lịch, không tự phát:** câu gỡ khỏi sổ yếu **phải** được xếp vào bảng *"🔁 Lịch kiểm tra lại"* trong [weak-register.md](14-prep/mock-interview/weak-register.md) (hạn = tuần gỡ + 2, kèm **góc đã dùng để cấm lặp**). Gỡ mà không xếp lịch = câu đó biến mất vĩnh viễn khỏi vùng được hỏi.
