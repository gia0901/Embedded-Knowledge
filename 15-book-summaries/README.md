...
- **Tài liệu tự chứa — cho phép lặp với topic 01–13.** Người đọc không phải nhảy sang tài liệu khác giữa chừng; giải thích đầy đủ tại chỗ, link tới file topic liên quan chỉ là *tùy chọn* đọc thêm. Khi nội dung hai nơi mâu thuẫn (do sửa một bên), **file topic 01–14 là chuẩn cho interview prep** — sửa book summary theo.
...
- **Đọc thêm (tùy chọn):** link file topic 01–13 liên quan.
```
...
- Với cụm 🎯: nhiều câu hỏi hơn, kèm **bẫy thường gặp khi trả lời**; link câu hỏi tương ứng trong bank nếu có.
...
| The Linux Programming Interface — Michael Kerrisk (No Starch 2010) | ✅ **Xong**; **đã đối chiếu PDF** | Thư mục `the-linux-programming-interface/`: README + **10 cụm phủ đủ 64 chương**. Offset trang xác minh 2 điểm: **trang PDF = trang sách + 44**. **6 cụm 🎯 đầu tư sâu** — 01 file I/O (3 bảng kernel, atomicity, 2 tầng buffer) · 02 process/exec (COW, số đo fork/vfork/clone) · 03 signals (async-signal-safe, bảng SA_RESTART, race với chính handler) · 04 threads (thread-safe vs reentrant, cond var, fork đa luồng) · 05 epoll (level vs edge, số đo hiệu năng) · 06 IPC (bảng chọn, PIPE_BUF, bẫy fcntl lock). 4 cụm tra cứu: 07 sockets (UNIX domain + SCM_RIGHTS + thiết kế server) · 08 mmap/soname/dlopen · 09 i-node/link/rename · 10 daemon/realtime/capabilities. **Trúng trụ số 1 của JD Datalogic** (*Linux userspace + debugging*) |
| **C++ High Performance** — Andrist & Sehr (2nd ed, Packt 12/2020) | ✅ **Xong (CHỌN LỌC 3/14 chương)**; **đã đối chiếu PDF** | Thư mục `cpp-high-performance/`: README + 3 file. Offset xác minh 3 điểm: **trang PDF = trang sách + 27**. **Cố ý chỉ làm ch. 3, 4, 7** — lý do ghi rõ ở README của thư mục: ch. 1–2 trùng *Effective Modern C++*, ch. 11 trùng *Concurrency in Action* (11 chương đã đọc sâu), và **ch. 6, 8, 12, 13, 14 là C++20** trong khi JD Datalogic là **C++17** (~30% cuốn sách không dùng được). **Giá trị riêng:** ch. 7 memory management (arena trên stack, alignment/padding, `std::pmr`) — repo trước đó chỉ có ~1 trang về PMR; ch. 4 biến 'cache locality' từ **một đoạn văn** ở [12-dsa §5] thành **số đo** (40 ms vs 800 ms; 11→4→0,7 ms). 21 câu Góc interview, mọi số đo đều là số thật sách in |
