...
- **Vị trí:** ✅ **XONG D1** + **1 phiên ngoài lịch** (`rapid resume` 18/08, quét sạch domain `RES`). Tiếp theo: **D2, nhưng đổi track** — xem ▶️ LÀM TIẾP.
...
- **Buổi gần nhất:** `rapid track resume` (**ngoài lịch D2** — người học chủ động chọn) — **3.67/4**, 12 câu, **9/12 đạt 4** ([log](../mock-interview/sessions/2026-08-18--rapid--resume.md)). 🎯 **`RES` đi từ 0% → 100% phủ trong MỘT phiên** — domain *chắc chắn 100% bị hỏi* nay đã quét hết. Câu 2đ duy nhất: **RES-012** (resume thiếu Yocto/I2C-SPI/PCI-USB) → vào sổ yếu.
...
- **Buổi trước đó:** `rapid track drivers-dt` (D1) — **3.92/4**, 11/12 đạt 4 ([log](../mock-interview/sessions/2026-08-17--rapid--drivers-dt.md)). *(B1: 3.25 · B2: 3.3 · B3: 2.83 · B4: 3.17 · B5: 3.67 · CN-weak: 3.0 · CN-comp: 3.0 · T2B1: 2.67 · T2B2: 2.1 · rapid-dbg: 3.67 · D1: **3.92** · rapid-resume: **3.67**)*
...
- **📊 ĐỘ PHỦ đo lại 18/08 (sau phiên):** **139/410 = 34%** *(17/08: 114/397 = 29%)*. `RES` **100%** ✅ · CPP 72% · LNX 60% · OS 39% · DBG 31% · DRV 30% · BUS 29% · BLD 26% · DP 21% · DSA 12% · SD 12% · **BSP 7%** 🔴 · **EMB 3%** · **NET 0%** · **BEH 0%**.
  - ⇒ **`BSP` vẫn là 🔴 số 1** (7%, trụ JD, chưa có bằng chứng độ vững). **`BEH` 0%** nay là rủi ro số 2 — nó **chắc chắn bị hỏi** như `RES`, và cũng là loại "lỗi đóng gói" mà phiên 18/08 vừa lộ ra.
- **▶️ LÀM TIẾP:** **`/mock rapid track bsp`** *(BSP 7% — ưu tiên số 1, chưa đo bao giờ)*. Đọc trước: [yocto.md](../../06-build-systems/yocto.md) ở mức tư duy + [boot-process.md](../../08-embedded-systems/boot-process.md).
  - Sau đó: **`/mock rapid track behavioral`** *(BEH 0% — cùng họ lỗi "đóng gói" với RES-012)*.
  - ⏰ **NỢ RETENTION — QUÁ HẠN 3 PHIÊN LIÊN TIẾP, PHẢI CHỐT LỊCH.** `CPP-009` · `DP-002` · `OS-003` · `OS-007` đến hạn **17–23/08**, bị bỏ qua ở cả 3 phiên 16, 17, 18/08 vì `rapid` không mang nổi follow-up mà retention cần. **Chúng cần một phiên `daily` riêng, không nhét vào rapid được** ⇒ chạy **`/mock daily track cpp-system`** với 4 câu này làm nòng cốt (góc mới đã soạn sẵn ở [weak-register](../mock-interview/weak-register.md)). Bỏ qua lần thứ tư = coi như đã xoá chúng khỏi vùng được hỏi.
  - 🗣️ **BÀI TỰ LUYỆN (không cần mock, làm ngay):** dựng khung **RES-012** theo công thức 3 bước, **nói to bấm giờ 60 giây**, lặp tới khi bật ra tự nhiên. Cùng cách với phần S-Box của **RES-001** (đặt *impact* ở 10 giây đầu) và **RES-011** (rút còn 45 giây + 1 câu bắc cầu).
- **Nợ lại:**
