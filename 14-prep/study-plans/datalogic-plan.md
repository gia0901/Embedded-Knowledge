...
- **Vị trí:** ✅ **XONG D1** + **4 phiên ngoài lịch** (`resume` 18/08 → RES 100% · `bsp` 19/08 → BSP 40% · `cpp-system` 21/08 → trả xong nợ retention · **`behavioral` 23/08 → BEH 0% → 100%**). 🎉 **KHÔNG CÒN DOMAIN TRẮNG NÀO.** Xem ▶️ LÀM TIẾP.
...
- **📊 ĐỘ PHỦ đo lại 23/08:** **161/414 = 39%** ⬆️ *(bank +3 câu `BEH-010/011/012`)*. **BEH 100%** 🆕 · RES 100% · CPP 72% · LNX 60% · BSP 40% · OS 39% · DBG 31% · COD 30% · DRV 30% · BUS 29% · BLD 26% · DP 20% · DSA 12% · SD 12% · **EMB 3%** · **NET 0%**. RES **100%** · CPP 72% · LNX 60% · **BSP 40%** ⬆️ *(7%)* · OS 39% · DBG 31% · COD 30% · DRV 30% · BUS 29% · BLD 26% · DP 21% · DSA 12% · SD 12% · **EMB 3%** · **NET 0%** · **BEH 0%**.
  - ⇒ ✅ **`BEH` đã lấp xong 23/08 (0% → 100%)** — nhưng lấp phủ **không** nghĩa là vững: điểm **2.42** là thấp nhất mọi phiên. **Rủi ro số 1 nay là `NET` 0%** *(12 câu — TCP/UDP, socket, framing; JD có mảng giao tiếp thiết bị)*. `EMB` 3% vẫn xếp sau vì phần lớn ngoài JD (bare-metal/MCU/RTOS) — [config §7](../mock-interview/config.md) cho phép cố ý để phủ thấp **nếu ghi rõ lý do**, và đây là lý do.
...
- **▶️ LÀM TIẾP:** 🗣️ **BÀI TỰ LUYỆN — KHÔNG PHẢI PHIÊN MOCK, và nay là việc GẤP NHẤT.** Viết **5 khung STAR** ra giấy, **mỗi khung kết bằng một con số**, rồi **nói to bấm giờ 60–90 giây/khung**. Năm khung: ① **thành tựu** (S-Box: *lệch nhau nhìn thành từng ô → mq đồng bộ → nhìn như một màn, một binary hai chế độ*) · ② **bug khó** (*đường bật bất đối xứng — đặt hai đường cạnh nhau*) · ③ **bất đồng** (chọn 1 trong 3: mq vs shm · cửa sổ 4 giây · sửa ở FRC hay lib) · ④ **học nhanh** (**Yocto/BeagleBone Black** — KHÔNG phải Windows app) · ⑤ **vì sao tuyển em** (3 trụ, mỗi trụ 1 bằng chứng). ⚠️ Bài này đã nợ **5 ngày** và phiên 23/08 chứng minh nó là nút thắt lớn nhất còn lại. **Làm xong mới chạy `/mock rapid track behavioral` lần 2 để đo lại.**
  ...
  - 🗣️ ⚠️ **Bài tự luyện nợ từ 18/08 — ĐÃ TRẢ GIÁ ngày 23/08, nay gộp vào ▶️ LÀM TIẾP ở trên.** Dựng khung `RES-012` + rút gọn `RES-001`/`RES-011`, nói to bấm giờ 60 giây. **Bằng chứng nó chưa được làm:** phiên `behavioral` 23/08 hỏng **y hệt** ở đúng ba chỗ đó — `BEH-001` lặp lại lỗi `RES-001` (thiếu impact), `BEH-011` lặp lại lỗi `RES-012` (không bắc cầu sang Yocto/BBB). Cùng một lỗi, đổi vỏ câu hỏi, **hai lần trong 5 ngày**.
- **Nợ lại:**
