...
- **Bối cảnh:** 🎯 Datalogic plan — **Tuần 1 Buổi 5**. Bài COD-006 (ring buffer) được chấm theo thoả thuận riêng: **không tính điểm phần tái tạo code** (ứng viên vừa đọc [12-dsa/ring-buffer.md](../../../12-dsa/ring-buffer.md) hôm trước), chỉ chấm **câu hỏi ngược, lựa chọn thiết kế, và giải thích**.
...
**(c) Trích tài liệu gốc — [12-dsa/ring-buffer.md §4](../../../12-dsa/ring-buffer.md):**
...
Và [§1](../../../12-dsa/ring-buffer.md), về đúng cái bản cuối vi phạm:
...
- **Lỗ hổng ưu tiên (top 3):**
  1. **Thiếu câu hỏi spec về đa luồng** — [ring-buffer.md §11 bước 1](../../../12-dsa/ring-buffer.md) + [§9 cây quyết định](../../../12-dsa/ring-buffer.md). Đọc lại đúng hai mục đó.
  2. **Không tổng quát hoá bug sau khi sửa** — sửa `pop()` nhưng để nguyên `empty()`/`full()` public không khoá. Sau khi vá một race, phải quét cả class xem còn chỗ nào cùng dạng: [ring-buffer.md §10 checklist](../../../12-dsa/ring-buffer.md).
  3. **Phản xạ "cứ dùng STL container"** trong đường dữ liệu nóng embedded — [ring-buffer.md §1 bảng so sánh](../../../12-dsa/ring-buffer.md) và [08/constraints.md](../../../08-embedded-systems/constraints.md) (cấm heap sau init).
