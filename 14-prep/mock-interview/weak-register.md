...
| OS-020 | thread-safe vs reentrant / async-signal-safe | cpp-system, os | 1 | **2** | 🆕 2026-08-13. Phân biệt đúng hai khái niệm + nhận đúng **self-deadlock** khi handler chen vào hàm đang giữ mutex. Nhưng **SAI** phần vận dụng: nói `write()` không dùng được trong signal handler (thực ra **write LÀ async-signal-safe**), và đề xuất họ `_r` như cách đạt signal-safety (`_r` giải quyết **thread-safety**, không phải signal-safety). Chưa có nguyên tắc tự suy *"hàm nào lấy khoá / đụng trạng thái toàn cục dở dang thì cấm"*. Bank [OS-020](bank/os.md) đã được viết lại kèm bảng tra + code. **Lần sau hỏi:** cho một handler gọi `malloc`/`printf` → *"cái nào cấm, vì sao, sửa bằng mẫu gì?"* (self-pipe trick) |
