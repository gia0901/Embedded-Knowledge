...
> 📑 Thứ tự theo **chủ đề** (A–G, BUS), không theo số ID — thêm câu mới đặt vào đúng mục ([vì sao](README.md#-id--vị-trí-trong-file)).

| Mục | Nội dung | Câu |
|---|---|---|
| **A** | Driver cơ bản, module & interrupt | 7 |
| **B** | Ranh giới kernel ↔ user space | 5 |
| **C** | Device Tree | 6 |
| **D** | Kiến trúc phần cứng: MCU/MPU, MMIO, DMA | 3 |
| **E** | PCI & USB | 9 |
| **F** | Boot, storage & OTA | 2 |
| **G** | Ràng buộc embedded & real-time | 3 |
| **BUS** | Bus Protocols (I2C / SPI / UART) | 7 |

---

## A — Driver cơ bản, module & interrupt
...
|---|---|
| **Xác thực địa chỉ thuộc user space** | User truyền vào một **địa chỉ KERNEL**. Driver ghi lên đó ⇒ user vừa ghi được vào bộ nhớ kernel *qua tay bạn* ⇒ **leo thang đặc quyền**. Đây là cả một lớp CVE có thật |
...
| **Trả số byte CHƯA chép** | Cho bạn đường thoát `-EFAULT`. Không có nó thì bạn xử lý tiếp trên **dữ liệu rác** mà không biết |

⇒ Vì việc thứ hai **có thể ngủ**, `copy_*_user` **cấm gọi trong ngữ cảnh atomic** (spinlock/ISR) — xem bẫy (3).
...
| Tái dùng được không | ✅ Chip TMP102 gắn ở board nào cũng chung driver | ❌ Board A và board B **không bao giờ** dùng chung |
| Thừa thì sao | **Vô hại** — không match `compatible` thì **không probe**, chỉ tốn ít dung lượng | **Boot hỏng** — kernel đi tìm thiết bị ở địa chỉ không có gì |

⇒ Nguyên tắc: **thứ tái dùng được thì gói chung; thứ riêng của từng board thì tách ra.**

Một kernel image chứa driver của 100 chip vẫn chạy đúng trên mọi board — chỉ driver khớp mới probe. Nhưng nhét mô tả board vào kernel thì image đó **chỉ còn chạy được trên đúng một board**, mất sạch lợi ích mà DT sinh ra để có.

📌 Chính xác hơn: DT **có thể** nhúng vào kernel image (vd `CONFIG_ARM_APPENDED_DTB`) — nhưng người ta **cố tình tránh**, đúng vì lý do trên. Không phải *"không thể"*, mà là *"không nên"*.
...
## D — Kiến trúc phần cứng: MCU/MPU, MMIO, DMA
...
3. Trong kernel Linux thì **không tự ép kiểu địa chỉ vật lý** — xem mục dưới.
...
| | MCU bare-metal | Kernel Linux |
|---|---|---|
| Địa chỉ | Vật lý, dùng thẳng | **Phải `ioremap`** — CPU chỉ nói được địa chỉ **ảo** |
| Địa chỉ lấy từ đâu | Hằng số trong datasheet | **Device tree** (`reg`) ⇒ cùng driver chạy nhiều board |
| Cách truy cập | Con trỏ `volatile` | **`readl()`/`writel()`** (hoặc `readb/w/q`) |
| Ai lo rào chắn bộ nhớ | Bạn tự thêm | **Đã nằm trong accessor** |

⭐ **Điểm đảo ngược thói quen MCU:** trong kernel bạn **không dùng `volatile` cho thanh ghi** — bạn dùng accessor. Chúng vừa chống tối ưu hoá, vừa có **memory barrier** (thứ `volatile` **không** cho), vừa xử lý endianness và khác biệt kiến trúc. Kernel có hẳn tài liệu nói `volatile` gần như luôn là dấu hiệu code sai.

> ⚠️ Tên hàm là **T3 (không chấm điểm)**. Thứ **được chấm** là hiểu **vì sao phải map**: có MMU nên địa chỉ vật lý vô nghĩa với CPU, và accessor mang theo rào chắn.
...
| | I2C / SPI | USB / PCI |
|---|---|---|
| Hỏi *"anh là ai?"* được không | ❌ **Không có lệnh chuẩn nào để hỏi** | ✅ Có, nằm trong chuẩn của bus |
| Ai nói cho kernel biết | **Device tree** — con người viết ra | **Chính thiết bị** tự khai |

⇒ DT tồn tại **không phải vì bus cũ kém**, mà vì chúng **không có kênh để tự giới thiệu**. Đây là lý do gốc DT tồn tại cho embedded bus.
</details>
...
|---|---|---|
| **SPI** | 2 (MOSI + MISO) | **Full** — mỗi nhịp clock đẩy ra 1 bit **và** nhận vào 1 bit, *đồng thời* |
| **I2C** | 1 (SDA) | **Half** — phải thay phiên chiều |
| **UART** | 2 (TX + RX) | **Full** |

⭐ **Hệ quả thực tế:** thao tác SPI đúng bản chất là **"trao đổi"**, không phải "gửi rồi nhận" — muốn *đọc*, bạn vẫn phải *gửi* ra byte rác để tạo clock. Nhờ đó ADC/màn hình SPI stream được liên tục: vừa đẩy lệnh khung sau vừa nhận dữ liệu khung trước, không có thời gian chết. **Giá:** thêm dây + một chân CS cho mỗi slave.
</details>
...
So sánh với ngõ ra **push-pull**: một chip đẩy lên 3,3 V trong khi chip kia kéo xuống 0 V ⇒ **không có điện trở nào ở giữa** ⇒ dòng chỉ bị giới hạn bởi trở kháng transistor ⇒ **hàng trăm mA tới vài A ⇒ cháy chip**.

> ⇒ **Open-drain + pull-up = KHÔNG BAO GIỜ có thể chập**, dù bao nhiêu thiết bị cùng nói một lúc. Cái giá là 0,7 mA, đổi lấy an toàn tuyệt đối trên dây dùng chung.
