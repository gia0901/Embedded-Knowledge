...
1. Đọc hợp đồng vận hành: `15_prep/mock-interview/config.md`, cùng `tracks.md`, `interview-types.md`, `weak-register.md` trong cùng thư mục (đường dẫn tương đối gốc repo Embedded-Interview).
2. Tham số phiên người dùng đưa (nếu có): **$ARGUMENTS**. Nếu thiếu track/type, hỏi ngắn gọn để chốt (mặc định theo config: track `bsp`, type `daily`, theo `daily-plan.md` nếu hôm nay có lịch).
3. Chạy đúng **giao thức phiên** trong config.md:
   ...
   - Câu coding: yêu cầu ứng viên viết vào `15_prep/mock-interview/coding-arena/`.
